...
class MFCC:
    ...
    def compute_mfcc(self, input_frame, include_dc=False):
        ...
        self.pre_frame = self.windowed_frame - self.alpha * np.concatenate(
            (np.full(1, 0), self.windowed_frame[:-1]))  # Preemphasis
        self.fft_frame = np.fft.rfft(self.pre_frame, self.N)  # Computing FFT
        self.mel_fb_sq_weights = self.generate_filter_bank() ** 2
        self.mel_frame = np.log(np.dot(self.mel_fb_sq_weights,
                                       (np.abs(self.fft_frame) ** 2)) + self.eps)  # Passing through mel and taking log
        self.mel_frame_normalized = self.mel_frame / np.sum(self.mel_fb_sq_weights,
                                                            axis=1)  # normalizing by filter energies
